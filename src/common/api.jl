...
function initialize!(g::MomentBasedEstimator{MDEstimator{M, V, S, T}}) where {M <: MomentFunction,V <: Divergence,S <: Constrained,T <: Weighting}
    n, p, m = size(g)
    ξ₀ = [ones(n); startingval(g)]
    g.e.gele = Int((n+p)*(m+1)-p)
    g.e.hele = Int(n*p + n + (p+1)*p/2)
...
function constrained(h::Function, hlb::Vector, hub::Vector, g::MomentBasedEstimator{T}) where {T <: GMMEstimator}
...
function constrained(h::Function, hlb::Vector, hub::Vector, g::MomentBasedEstimator{T}) where {T <: MDEstimator}
    p = npar(g); chk = check_constraint_sanity(p, startingval(g), h, hlb, hub)
    r  = MomentBasedEstimatorResults(:Uninitialized, 0., Array{Float64}(p), Array{Float64}(p, p))
...
solve!(g::MomentBasedEstimator{S}, s::MathProgBase.SolverInterface.AbstractNonlinearModel) where {S <: MDEstimator} = MathProgBase.optimize!(g.m)
